...
var upload = multer({
  storage: multerS3({
    ...
    key: function(req, file, cb) {
      var path = `products/${file.originalname.replace(
        '.jpg',
        ''
      )}_${Date.now()}.jpg`;
      cb(null, path);
...
var imageLoad = upload.fields([
  { name: 'products_main_image', maxCount: 1 },
  { name: 'products_images', maxCount: 3 }
]);
router.post('/admin/product.html', imageLoad, (req, res) => {
  ...
  const sizes = JSON.stringify(
    req.body.products_sizes.replace(/\s+/g, '').split(',')
  );
  const variants_code = req.body.products_variants_code
    .replace(/\s+/g, '')
    .split(',');
  const variants_name = req.body.products_variants_name
    .replace(/\s+/g, '')
    .split(',');
  const variants_size = req.body.products_variants_size
    .replace(/\s+/g, '')
    .split(',');
  const variants_stock = req.body.products_variants_stock
    .replace(/\s+/g, '')
    .split(',');
  ...
  db.query(sql_insert_product, products, (err, result) => {
    ...
    for (let i = 0; i < variants_code.length; i++) {
      ...
      let variants = {
        color_code: variants_code[i],
        name: variants_name[i],
        size: variants_size[i],
...
var campaigns = multer({
  storage: multerS3({
    ...
    key: function(req, file, cb) {
      var path = `campaigns/${file.originalname.replace(
        '.jpg',
        ''
      )}_${Date.now()}.jpg`;
      cb(null, path);
  ...
  async.waterfall(
    [
      ...
      function(rst1, rst2, rst3, next) {
        db.query(
          `SELECT user_id FROM token LEFT JOIN user ON token.user_id = user.id WHERE user.id=${rst2.insertId}`,
          (err4, result4) => {
            next(err4, result4);
          }
        );
...
router.post('/admin/signin', (req, res) => {
  ...
  if (provider == 'native') {
    ...
    async.waterfall(
      [
        function(next) {
          ...
          db.query(
            `SELECT u.* FROM user AS u WHERE u.provider = '${provider}' AND u.email = '${email}' AND u.password ='${password}'`,
            (err1, result1) => {
              next(err1, result1);
            }
          );
    ...
    request(
      `https://graph.facebook.com/v3.3/${fb_id}?access_token=${fb_token}&fields=name, email,picture{url}`,
      ...
      async.waterfall(
        [
          function(next) {
            // 搜尋使用 FB 註冊過
            db.query(
              `SELECT email, id FROM user WHERE provider = 'facebook' AND email = '${email}';`,
              (err1, result1) => {
                next(err1, result1);
              }
            );
          ...
          function(rst1, next) {
            if (rst1.length == 0) {
              ...
              db.query(
                ...
                { provider: 'facebook', name, email, picture },
                (err2, result2) => {
                  db.query(
                    `SELECT id FROM user WHERE email = '${email}' AND provider = 'facebook'`,
                    (err2_1, result2_1) => {
              ...
              // 若有則 update user name & picture
              var id = rst1[0].id;
              db.query(
                `UPDATE user SET name = '${name}',picture='${picture}' WHERE user.email = '${email}' AND user.provider = 'facebook'`,
                (err2, result2) => {
                  next(err2, result2, id);
                }
              );
