...
const men_link = document.getElementsByClassName('nav-link')[1];
const urlParams = new URLSearchParams(window.location.search);
const paging = urlParams.get('paging');
attachBannerList(carousel);
attachMenList(product_container);
attachPaging(page);
...
function attachMenList(product_container) {
    const brown_color = '#8b572a'
    men_link.style.color = brown_color;
    var req = new XMLHttpRequest();
    req.onreadystatechange = function () {
        if (req.readyState === 4) {
            var res = JSON.parse(req.responseText);
        ...
        //當 user 輸入無效 product id 時，導入 index.html 頁面
            ...
            window.location.href = 'men.html?paging=0';
        }
    };
    req.open('get', `/api/1/products/men?paging=${paging}`);
    req.send();
}
function attachPaging(page) {
    ...
    req.onreadystatechange = function () {
        if (req.readyState == 4) {
            ...
            for (let i = 0; i < Math.ceil(count / show_num); i++) {
                ...
                page_link.href = `men.html?paging=${i}`;
    ...
    req.open('get', '/api/1/products/getCount?category=men');
    req.send();
